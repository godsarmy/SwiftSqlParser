...
public struct ExpressionDeparser {
    ...
    public func deparse(_ expression: any Expression) -> String {
        ...
        if let isNull = expression as? IsNullExpression {
            return isNull.isNegated ? "\(deparse(isNull.expression)) IS NOT NULL" : "\(deparse(isNull.expression)) IS NULL"
        }
        ...
        if let between = expression as? BetweenExpression {
            ...
            return "\(deparse(between.expression)) \(keyword) \(deparse(between.lowerBound)) AND \(deparse(between.upperBound))"
        }
        ...
        if let caseExpression = expression as? CaseExpression {
            ...
            for whenClause in caseExpression.whenClauses {
                components.append("WHEN \(deparse(whenClause.condition)) THEN \(deparse(whenClause.result))")
            }
