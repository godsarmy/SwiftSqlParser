...
func createTableParsesIntoCreateTableStatement() throws {
    let parsed = try parseStatement("CREATE TABLE users (id INT PRIMARY KEY, name TEXT DEFAULT 'anon' NOT NULL, role_id INT REFERENCES roles (id), CONSTRAINT users_name_check CHECK (name <> ''), PRIMARY KEY (id), FOREIGN KEY (role_id) REFERENCES roles (id))")
...
func alterTableParsesAddAndDropColumn() throws {
    ...
    let renameColumnParsed = try parseStatement("ALTER TABLE users RENAME COLUMN email TO primary_email")
    guard let renameColumn = renameColumnParsed as? AlterTableStatement else {
        Issue.record("Expected AlterTableStatement")
        return
    }
    if case let .renameColumn(oldName, newName) = renameColumn.operation {
    ...
    if case let .renameTable(newName) = renameTable.operation {
        #expect(newName == "app_users")
    } else {
        Issue.record("Expected rename table operation")
    }

    let addConstraintParsed = try parseStatement("ALTER TABLE users ADD CONSTRAINT users_pk PRIMARY KEY (id)")
    guard let addConstraint = addConstraintParsed as? AlterTableStatement else {
        Issue.record("Expected AlterTableStatement")
        return
    }
    if case let .addConstraint(constraint) = addConstraint.operation {
    ...
    if case let .dropConstraint(name) = dropConstraint.operation {
...
func deparserHandlesDdlStatements() {
    ...
    let create = CreateTableStatement(
        ...
        columns: [
            ...
            TableColumnDefinition(name: "name", typeName: "TEXT", defaultExpression: StringLiteralExpression(value: "anon"), constraints: [.notNull])
        ],
        constraints: [
            TableConstraintDefinition(kind: .check(RawExpression(sql: "name <> ''")))
        ]
    )
    #expect(deparser.deparse(create) == "CREATE TABLE users (id INT PRIMARY KEY, name TEXT DEFAULT 'anon' NOT NULL, CHECK (name <> ''))")

    let createIndex = CreateIndexStatement(name: "users_name_idx", table: "users", columns: ["name"], isUnique: true)
    ...
    let createView = CreateViewStatement(
        ...
        select: PlainSelect(
            ...
            whereExpression: BinaryExpression(left: IdentifierExpression(name: "active"), operator: .equals, right: NumberLiteralExpression(value: 1))
        )
    )
    #expect(deparser.deparse(createView) == "CREATE VIEW active_users AS SELECT id FROM users WHERE active = 1")

    let alter = AlterTableStatement(
        table: "users",
        operation: .addColumn(TableColumnDefinition(name: "email", typeName: "TEXT", defaultExpression: StringLiteralExpression(value: "none")))
    )
    #expect(deparser.deparse(alter) == "ALTER TABLE users ADD COLUMN email TEXT DEFAULT 'none'")

    let renameColumn = AlterTableStatement(table: "users", operation: .renameColumn(oldName: "email", newName: "primary_email"))
    #expect(deparser.deparse(renameColumn) == "ALTER TABLE users RENAME COLUMN email TO primary_email")
    ...
    let addConstraint = AlterTableStatement(
        table: "users",
        operation: .addConstraint(TableConstraintDefinition(name: "users_pk", kind: .primaryKey(columns: ["id"])))
    )
    #expect(deparser.deparse(addConstraint) == "ALTER TABLE users ADD CONSTRAINT users_pk PRIMARY KEY (id)")
...
func createViewParsesIntoDedicatedNode() throws {
    let viewParsed = try parseStatement("CREATE VIEW active_users AS SELECT id FROM users WHERE active = 1")
